...
const characterStore = module.exports = {
   ...
   *
   * @param {string} args.user - The current user's ID
   * @param {string} [args.id] - The ID of the character to delete
   * @param {string} [args.normalizedName] - The normalized name of the character to delete
   * @returns {Promise} - Resolves without a return value
   */
  deleteAll ({ user }) {
    return characterStore.findOne({ user, id, normalizedName })
      .then(character => {
        if (['sidekick', 'nemesis'].includes(character.type)) {
          //
        }
        let query = {
          TableName,
        ...
        return util.promisify(dynamoDB, 'delete', query);
      })

      // The character doesn't exist, which is ok
  ...
  findOrCreate ({ user, character }) {
    if (character.id || character.normalizedName) {
      return characterStore.findOne(Object.assign({ user }, character));
    }
    else {
      return characterStore.create({ user, character });
    }
   ...
   *
   * @param {string} args.user - The current user's ID
   * @param {string} [args.id] - The ID of the character to find
   * @param {string} [args.normalizedName] - The normalized name of the character to find
   * @returns {Promise<Character>} - Resolves with the existing or newly-created character
   */
  findOne ({ user, id, normalizedName }) {
    return Promise.resolve()
      .then(() => {
        ...
        if (normalizedName) {
          validate.nonEmptyString('normalizedName', normalizedName);
        }
