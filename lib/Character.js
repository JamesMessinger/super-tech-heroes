...
class Character {
  constructor ({ id, name, normalizedName, type, powers, weakness, bio, user, expires, sidekick, nemesis }) {
    ...
    this.sidekick = Character.fromReference({ reference: sidekick, type: "sidekick" });
    this.nemesis = Character.fromReference({ reference: nemesis, type: "villain" });
  }
  ...
  static fromHierarchy (hierarchy) {
    ...
    hero.sidekick = Character.fromReference({ reference: sidekick, type: "sidekick" });
    hero.nemesis = Character.fromReference({ reference: nemesis, type: "villain" });
   ...
   *
   * @param {string|object} args.reference - The related character object, ID, or URL
   * @param {string} args.type - The type of relation (e.g. "sidekick", "nemesis")
   * @returns {?Character}
   */
  static fromReference ({ reference, type }) {
    if (reference === undefined || reference === null) {
      return null;
    }
    else if (typeof reference === "object") {
      return new Character(Object.assign({ type }, reference));
    }
    else if (typeof reference === "string") {
      if (validate.isGuid(reference)) {
        return new Character({ id: reference, type });
      }
      else {
        return Character.fromUrl({ url: reference, type });
      }
    }
    else {
  ...
  /**
   * Creates a {@link Character} object from a URL (e.g. "https://heroes.jamesmessinger.com/characters/supercoder").
   * Only the {@link Character#normalizedName} property will be set.
   *
   * @param {string} args.url - The character URL
   * @param {string} args.type - The character type (e.g. "sidekick", "nemesis")
   * @returns {Character}
   */
  static fromUrl ({ url, type }) {
    validate.nonEmptyString(type, url);
  ...
  validate ({ model = false, resource = false } = {}) {
    ...
    if (this.weakness !== undefined) {
      validate.string("weakness", this.weakness);
      validate.maxLength("weakness", this.weakness, 50);
    }

    if (this.bio !== undefined) {
      validate.string("bio", this.bio);
