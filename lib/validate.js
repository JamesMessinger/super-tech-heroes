...
let validate = module.exports = {
  ...
  /**
   * Throws an error unless the specified value a string
   *
   * @param {string} field - The name of the field that's being validated
   * @param {*} value - The value to validate
  ...
  /**
   * Throws an error unless the specified value is an array.
   *
   * @param {string} field - The name of the field that's being validated
   * @param {*} value - The value to validate
  ...
  array (field, value) {
    if (!Array.isArray(value)) {
      throw Response.badRequest(`The "${field}" value must be an array`);
    }
  ...
  /**
   * Throws an error unless the specified value is an object containing the character fields
   * that a user is allowed to set.
   *
   * @param {string} field - The name of the field that's being validated
   * @param {*} value - The value to validate
   * @returns {object} - Returns the character POJO
   */
  characterPOJO (field, value) {
    return validate.object(field, value, ['name', 'type', 'powers', 'weakness', 'bio', 'sidekick', 'nemesis']);
  },

  /**
   * Throws an error unless the specified value is an object containing only the specified properties
   * or a subset of those properties.
   *
   * @param {string} field - The name of the field that's being validated
   * @param {*} value - The value to validate
   * @param {string[]} allowedProperties - The allowed properties
   * @returns {object} - Returns the object
   */
  object (field, value, allowedProperties) {
    if (!value) {
      throw Response.badRequest(`The ${field} is missing`);
    }
    if (typeof value !== 'object') {
      throw Response.badRequest(`The ${field} must be an object`);
    }

    for (let key of Object.keys(value)) {
      if (!allowedProperties.includes(key)) {
        throw Response.badRequest(`Unknown field: "${key}"`);
      }
    }
