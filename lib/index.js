...
exports.handler = (request, context, callback) => {
  Promise.resolve()
    ...
    .then(({ handler, params }) => handler({ request, context, params }))
    .catch(error => {
 ...
 * @returns {{ handler: function, params: string[] }} - Resolves with the handler function and the path parameters
 */
function findHandler ({ path, httpMethod }) {
  let matchedRoute, params;
  ...
  let handler = matchedRoute[httpMethod];
  if (!handler) {
